...
use parent qw(Exporter CAF::Logger);
...
sub getLocks
{
    ...
    my $fl = CAF::Lock->new("$cache_root/$FETCH_LOCK_FN");
    $fl->set_lock($self->{LOCK_RETRIES}, $self->{LOCK_WAIT}, FORCE_IF_STALE) or
	die "Failed to lock $cache_root/$FETCH_LOCK_FN";
...
sub retrieve
{
	...
	my ($author, $payload) = _gss_decrypt($cnt);
...
sub download
{
    ...
    my $cache = sprintf("%s/data/%s", $self->{CACHE_ROOT}, EncodeURL($url));
